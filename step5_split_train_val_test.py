...
# Content: 第5步，將資料中原始資料JPEGImages中的圖檔，拆分為訓練、驗證集和測試集(採不重複抽樣)，
#          且製作三個資料集各自之圖片檔名文字清單(train.txt、valid.txt、test.txt)，並將各自資料集之圖檔和xml標籤檔
# 			分開複製到各自資料夾。
###
...
train_datasets_qty = 20224  # 訓練集資料挑選幀數
val_datasets_qty = 2528 # 驗證集資料挑選幀數
train_labels_path = './labels_train'    # 訓練資料集標記檔存儲路徑
val_labels_path = './labels_val'    # 驗證資料集標記檔存儲路徑
test_labels_path = './labels_test'  # 測試資料集標記檔存儲路徑
''' Edit Here End '''
...
ValidationSet = [files.pop(random.randrange(len(files))) for _ in range(val_datasets_qty)] #利用pop指令抽出已取出的訓練樣本，避免重複採樣
TestingSet = files #剩餘的圖檔為驗證資料
...
for file_names in TestingSet:
	shutil.copyfile(os.path.join(labels_path, file_names), os.path.join(test_labels_path, file_names))
...
# Testing Datasets
fileObject = open(os.path.join(data_list_file_save_path, model_name + '_test.txt'), 'w') #開啟新檔
for TestingSet_names in TestingSet:
    jpg_name = TestingSet_names.replace(".txt", ".jpg")
    fileObject.write('./' + model_name + '/images/' + jpg_name)
    fileObject.write('\n')
fileObject.close() #寫入完畢關檔